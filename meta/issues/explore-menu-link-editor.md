...
- **Layout integrity**: the section entries become keyed on the link text. The add/edit/remove controls must not break the icon-only collapse at `max-width: 1180px` (labels hidden, links centered) — per-link edit/remove hide in that mode (the section "+" may remain as it is icon-only); long saved queries clamp with ellipsis like other sidebar labels.
...
- The section "+" control stays visible in the ≤1180px icon-only collapse; per-link edit/remove are hidden there (icon-only rows have no room), matching the old issue's remove-control decision.
- Estimated effort: small — ~100–150 lines across `App.svelte` and `style.css`, one new smoke test; no backend, schema, or fixture work. Status: open.