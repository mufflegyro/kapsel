...
func TestRunImportSubscriptionsCommand(t *testing.T) {
	t.Parallel()
	...
	code := runWithConfig(context.Background(), cfg, []string{"import-subscriptions", csvPath}, strings.NewReader(""), &stdout, &stderr)
	if code != 0 {
		t.Fatalf("expected exit code 0, got %d stderr=%q", code, stderr.String())
	}
	var report subsimport.Report
	if err := json.NewDecoder(&stdout).Decode(&report); err != nil {
		t.Fatal(err)
	}
	if report.Parsed != 2 || report.Enqueued != 2 {
		t.Fatalf("unexpected import report: %#v", report)
	}
